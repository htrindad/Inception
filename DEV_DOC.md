...
## Architecture
- The browser sends an HTTP request to nginx (with TLS enabled), from which it will request the wordpress page (with FastCGI), and save it's contents in MariaDB.
...
To build and run this project. You must edit the `localhost` on your `/etc/hosts` file, to `<your_intra_name>`
...
- nginx gets their information from port 443 and encrypting it through TLS 1.2/1.3 (It generates a self-signed certificate). And `fast_cgi` pass to `wp-php:9000`.
...
- MariaDB entrypoint inits the database, and the users on the first run, then executes mariadb.
